...
function loadVersionFromCsv() {
  ...
  latestEnvRow=$(awk -F ',' -v env="$env" '$3 == env {latest=$0}END{print latest}' "$releasesCsv")
  latestRow=$(awk -F ',' '{latest=$0}END{print latest}' "$releasesCsv")
...
function computeNextVersion () {
  ...
  # Going from production to staging
