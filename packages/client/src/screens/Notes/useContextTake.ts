/**
 * The take played through its own AudioContext — the older of two ways.
 *
 * Kept for binaries whose engine cannot hold recorded audio. Bundles ship
 * over the air to builds made before the sample voice existed, and playback
 * there must degrade to what it was rather than to silence (INV-NOTES-030).
 * Everything else uses `useTakeVoice`, which puts the take on the one clock
 * (INV-NOTES-133).
 *
 * Decodes and plays a note's audio URI through react-native-audio-api's
 * `AudioContext`. State is local to one caller and entirely separate from the
 * live recording path.
 *
...
export function useContextTake({
  resolveAudioUri
