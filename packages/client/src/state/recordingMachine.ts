 ...
 * screen via `machine.provide(...)` — see the injection points below for the
 * action keys a consumer must supply.
 *
 * States:
...
/**
 * The machine. Side-effecting actions are referenced by name here and supplied
 * by the screen via `.provide`. Keep this file free of any audio/navigation
 * imports so it can be unit-tested with no device.
 *
...
export const recordingMachine = setup({
  ...
  actions: {
    setHandle: assign({
      handle: ({ event }) => (event.type === 'ANALYZED' ? event.data : null),
      errorMessage: () => null
    }),
    setError: assign({
      errorMessage: ({ event }) =>
  ...
  on: {
    // A failure can interrupt any state. Root-level targets are dot-prefixed
    // in v5 because they resolve relative to the state node they sit on.
  ...
  states: {
    ...
    analyzing: {
      entry: ['engineStop'],
      on: { ANALYZED: { target: 'result', actions: ['setHandle'] } }
    },
    result: {
      entry: ['onResult'],
      on: { RESET: 'idle' }
